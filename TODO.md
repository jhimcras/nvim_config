...
- [ ] 기존 버퍼에 실행 결과를 다시 Display 할때 버퍼가 Hidden 상태면 현재 tab에 다시 split(or vsplit) 하여 나타나도록 함.
- [ ] 동일 prjroot의 다른 버퍼에서도 launcher 버퍼를 제거 할 수 있는 단축키
- [ ] `vim.loop.kill(pid, 15)`를 사용 하는 것은 `read_stop`등을 할 수 없기 떄문에 handle을 사용하는게 좋다고 함. 리팩토링 필요.
